...
Considering the fact that we did not have a tree structure we are satisfied with the results of our local optimizations. It was unexpected that a trivial 3-opt would have time to run on larger instances, but as it turns out we got our best results when using it for all tour lengths.
