...
At this point the section from B to C should be reversed, so that it is traversed from C to B instead, in order to retain the rest of the tour configuration.
...
\subsection{3-opt}
3-opt works similarly to two opt, the difference being that three edges are chosen for swapping.
...
\subsection{Opts}
blablabla 2-opt = O($n^2$), 3-opt = O($n^3$)
...
We would have liked to try\dots\\
